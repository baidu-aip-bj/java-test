...
public class AipBodyAnalysis extends BaseClient {
    ...
    /**
     * 人体关键点识别接口   
     * 对于输入的一张图片（可正常解码，且长宽比适宜），**检测图片中的所有人体，输出每个人体的21个主要关键点，
     * 包含头顶、五官、脖颈、四肢等部位，同时输出人体的坐标信息和数量**。
     *
     * @param image - 二进制图像数据
     ...
     * @return JSONObject
     */
    public JSONObject handAnalysis(byte[] image, HashMap<String, String> options) {
        AipRequest request = new AipRequest();
        preOperation(request);

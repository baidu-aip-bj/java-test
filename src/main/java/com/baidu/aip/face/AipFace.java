...
public class AipFace extends BaseClient {
     ...
     * @param imageType - 图片类型     <br> **BASE64**:图片的base64值，base64编码后的图片数据，编码后的图片大小不超过2M；
     *                 <br>**URL**:图片的 URL地址( 可能由于网络等原因导致下载图片时间过长)； <br>**FACE_TOKEN**:
     ...
     *                ,quality,eye_status,emotion,face_type信息**  <br> 逗号分隔. 默认只返回face_token、人脸框、概率和旋转角度
     *   max_face_num 最多处理人脸的数目，默认值为1，仅检测图片中面积最大的那个人脸；**最大值10**，检测图片中面积最大的几张人脸。
     *   face_type 人脸的类型 **LIVE**表示生活照：通常为手机、相机拍摄的人像图片、或从网络获取的人像图片等**IDCARD**表示身份证芯片照
     *                ：二代身份证内置芯片中的人像照片 **WATERMARK**表示带水印证件照：一般为带水印的小图，如公安网小图
     *                **CERT**表示证件照片：如拍摄的身份证、工卡、护照、学生证等证件图片 默认**LIVE**
     *   liveness_control 活体检测控制  **NONE**: 不进行控制 **LOW**:较低的活体要求(高通过率 低攻击拒绝率) **NORMAL**:
     *               一般的活体要求(平衡的攻击拒绝率, 通过率) **HIGH**: 较高的活体要求(高攻击拒绝率 低通过率) **默认NONE**
     * @return JSONObject
     */
     ...
     *               <br>**此阈值设置得越高，检索速度将会越快，推荐使用默认阈值`80`**
     ...
     *
     * @param image - 图片信息(**总数据大小应小于10M**)，图片上传方式根据image_type来判断
     * @param imageType - 图片类型     <br> **BASE64**:图片的base64值，base64编码后的图片数据，
     *                  编码后的图片大小不超过2M； <br>**URL**:图片的 URL地址( 可能由于网络等原因导致下载图片时间过长)；
     *                  <br>**FACE_TOKEN**: 人脸图片的唯一标识，调用人脸检测接口时，会为每个人脸图片赋予一个唯一的FACE_TOKEN，
     *                  同一张图片多次检测得到的FACE_TOKEN是同一个。
     * @param groupIdList - 从指定的group中进行查找 用逗号分隔，**上限20个**
     * @param options - 可选参数对象，key: value都为string类型
     * options - options列表:
     ...
     *   match_threshold 匹配阈值（设置阈值后，score低于此阈值的用户信息将不会返回） 最大100 最小0 默认80
     *               <br>**此阈值设置得越高，检索速度将会越快，推荐使用默认阈值`80`**
     *   quality_control 图片质量控制  **NONE**: 不进行控制 **LOW**:较低的质量要求 **NORMAL**: 一般的质量要求 **HIGH**:
     *               较高的质量要求 **默认 NONE**
     *   liveness_control 活体检测控制  **NONE**: 不进行控制 **LOW**:较低的活体要求(高通过率 低攻击拒绝率)
     *                **NORMAL**: 一般的活体要求(平衡的攻击拒绝率, 通过率) **HIGH**:
     *                较高的活体要求(高攻击拒绝率 低通过率) **默认NONE**
     ...
     *                则会用新图替换库中该user_id下所有图片,默认使用APPEND
     * @return JSONObject
     */
     ...
     *              parsing信息，逗号分隔，默认只返回face_token、活体数、人脸框、概率和旋转角度。
     * @return JSONObject
     */
     ...
     * @return JSONObject
     */
    public JSONObject faceMingJingVerify(String idCardNumber, String name, String image, 
     ...
     *  image - 图片信息(数据大小应小于10M 分辨率应小于1920*1080)，5.2版本SDK请求时已包含在加密数据data中，无需额外传入
     *  imageType - 图片类型
     *  registerImage - 图片信息(总数据大小应小于10M)，图片上传方式根据image_type来判断。本图片特指客户服务器上传图片，非加密图片Base64值
     *  registerImageType - 图片类型
     * @param options - 可选参数对象，key: value都为string类型
     * @return JSONObject
     */
    public JSONObject faceMingJingMatch(String image, String imageType, String registerImage, String registerImageType, 
                                  HashMap<String, Object> options) {
        AipRequest request = new AipRequest();
        preOperation(request);
        ...
        request.setUri(FaceConsts.FACE_MATCH_V4);
        postOperation(request);
        return requestServer(request);
    }
     ...
     * @return JSONObject
     */
    public JSONObject onlinePictureLiveV4(String sdkVersion, 
                                  HashMap<String, Object> options) {
        AipRequest request = new AipRequest();
        preOperation(request);
        ...
        request.setUri(FaceConsts.ONLINE_PICTURE_LIVE_V4);
        postOperation(request);
        return requestServer(request);
    }

...
public abstract class BaseClient {
    ...
    private final int SOCKET_TIMEOUT_MILLIS = 20000;
    ...
    protected void preOperation(AipRequest request) {
        ...
        if (config == null) {
            config = new AipClientConfiguration();
            config.setConnectionTimeoutMillis(CONNECTION_TIMEOUT_MILLIS);
            config.setSocketTimeoutMillis(SOCKET_TIMEOUT_MILLIS);
        }
    ...
    protected void preOperation(AipRequest request, String httpContentType) {
        ...
        request.setHttpMethod(HttpMethodName.POST);
        request.addHeader(Headers.CONTENT_TYPE, httpContentType);
        request.addHeader("accept", "*/*");
        request.setConfig(config);
    }
    ...
    protected JSONObject requestServer(AipRequest request) {
        ...
        if (status.equals(200) && !resData.equals("")) {
            ...
            LOGGER.warn(String.format("call failed! response status: %d, data: %s, error msg：%s",
                    status, resData, jsonObject.toString()));
